...
*   **Key Tasks:**
    ...
    *   `[~]` **Implement On-Demand Printing for Windows Companion App (See `docs/PrinterArchitecture.md#6-on-demand-printing-for-windows-companion-app`)**
        ...
        *   `[ ]` **Windows Companion App (`SagraFacile.WindowsPrinterService`):**
            *   `[ ]` **`SignalRService.cs`:**
                ...
                *   `[ ]` Implement in-memory `ConcurrentQueue<PrintJobItem>`.
                *   `[ ]` Handle incoming jobs: queue if `OnDemandWindows`, print if `Immediate`.
                *   `[ ]` Expose methods to dequeue jobs and get queue count. Raise `OnDemandQueueCountChanged` event.
            *   `[ ]` **`PrintStationForm.cs` (New Form):**
                *   `[ ]` UI: Pending count label, "Print Next" button, optional activity log.
                *   `[ ]` Logic: Get initial count, subscribe to queue changes, dequeue/print job via `SignalRService` and `IRawPrinter`.
            *   `[ ]` **`ApplicationLifetimeService.cs`:**
                *   `[ ]` Add tray menu item to open `PrintStationForm`.
                *   `[ ]` Manage `PrintStationForm` instance and dependencies.
