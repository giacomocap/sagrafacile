...
namespace SagraFacile.NET.API.Services.Interfaces
{
    public interface ICashierStationService
    {
        ...
        Task<IEnumerable<CashierStationDto>> GetStationsByOrganizationAsync(Guid organizationId, User currentUser);
        Task<IEnumerable<CashierStationDto>> GetStationsByAreaAsync(int areaId, User currentUser);
        Task<(CashierStationDto? Station, string? Error)> CreateStationAsync(Guid organizationId, CashierStationUpsertDto dto, User currentUser);
